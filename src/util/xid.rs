const XID_START_RANGES: &[(usize, usize)] = &[
...
const XID_CONTINUE_RANGES: &[(usize, usize)] = &[
...
pub fn is_xid_start(c: char) -> bool {
    for (s, e) in XID_START_RANGES {
        if *s <= (c as usize) && (c as usize) <= *e {
            return true;
        }
    }
    false
...
pub fn is_xid_continue(c: char) -> bool {
    ...
    for (s, e) in XID_CONTINUE_RANGES {
        if *s <= (c as usize) && (c as usize) <= *e {
            return true;
        }
    }
